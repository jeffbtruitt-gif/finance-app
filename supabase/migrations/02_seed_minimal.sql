...
begin
  ...
  insert into tf_categories (household_id, scheme_id, name, group_name, sort_order) values
    (v_household_id, v_scheme_id, 'Rent',         'Rent & Utilities', 10),
    (v_household_id, v_scheme_id, 'Electric',     'Rent & Utilities', 20),
    (v_household_id, v_scheme_id, 'Gas',          'Rent & Utilities', 30),
    (v_household_id, v_scheme_id, 'Water',        'Rent & Utilities', 40),
    (v_household_id, v_scheme_id, 'Internet',     'Rent & Utilities', 50),
    (v_household_id, v_scheme_id, 'Phone',        'Rent & Utilities', 60);

  -- Food & Car
  insert into tf_categories (household_id, scheme_id, name, group_name, sort_order) values
    (v_household_id, v_scheme_id, 'Groceries',        'Food & Car', 110),
    (v_household_id, v_scheme_id, 'Restaurants',      'Food & Car', 120),
    (v_household_id, v_scheme_id, 'Transportation',   'Food & Car', 130),
    (v_household_id, v_scheme_id, 'Auto Maintenance', 'Food & Car', 140);

  -- Other (general)
  insert into tf_categories (household_id, scheme_id, name, group_name, sort_order) values
    (v_household_id, v_scheme_id, 'Shopping',       'Other', 210),
    (v_household_id, v_scheme_id, 'Entertainment',  'Other', 220),
    (v_household_id, v_scheme_id, 'Health',         'Other', 230),
    (v_household_id, v_scheme_id, 'Kids',           'Other', 240),
    (v_household_id, v_scheme_id, 'Personal Care',  'Other', 250),
    (v_household_id, v_scheme_id, 'Gifts',          'Other', 260),
    (v_household_id, v_scheme_id, 'Travel',         'Other', 270),
    (v_household_id, v_scheme_id, 'Miscellaneous',  'Other', 280);

  -- Yearly
  insert into tf_categories (household_id, scheme_id, name, group_name, sort_order, is_yearly) values
    (v_household_id, v_scheme_id, 'Insurance',     'Yearly', 310, true),
  ...
  select id into v_utilities_id   from tf_categories where scheme_id = v_scheme_id and name = 'Electric';
  ...
  select t.id, v_scheme_id,
    case
      when t.description like 'TARGET%' or t.description like 'AMAZON%'
        or t.description like 'COSTCO%' or t.description like 'HOME DEPOT%' then v_shopping_id
